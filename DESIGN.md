...
| Hand-rolled ZIP reading foreign archives | Closed. Verified byte-exact against 7-Zip, libarchive and Python `zipfile`, stored and deflated, plus an archive comment displacing the EOCD. The guard that matters is reading each **local** header's own extra-field length instead of the central directory's: 7-Zip writes 36 bytes centrally and 0 locally, libarchive writes 24 centrally and 32 locally, so the two diverge in opposite directions and either assumption alone misreads every entry. Sizes come from the central directory, which is also what makes libarchive's data-descriptor entries (flag `0x0008`, zeroed local sizes) work. An unsupported method — 7-Zip's LZMA option — is refused by name rather than misread. |
